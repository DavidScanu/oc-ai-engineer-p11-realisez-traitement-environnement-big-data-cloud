#!/bin/bash
# Script de vérification de la configuration avant création du cluster
...
echo "=================================================="
echo "🔍 VÉRIFICATION DE LA CONFIGURATION - ÉTAPE 1"
echo "=================================================="
echo ""
...
SAMPLE_COUNT=$(aws s3 ls "${S3_DATA_INPUT}" --recursive --region "${AWS_REGION}" | grep "\.jpg$" | wc -l)
if [ "${SAMPLE_COUNT}" -gt 0 ]; then
    echo "   ✅ Données trouvées: ~${SAMPLE_COUNT} fichiers .jpg"
else
    echo "   ⚠️  WARNING: Aucun fichier .jpg trouvé dans ${S3_DATA_INPUT}"
...
if aws s3 ls "${S3_SCRIPTS}read_fruits_data.py" --region "${AWS_REGION}" &>/dev/null; then
    echo "   ✅ read_fruits_data.py présent"
else
    echo "   ❌ read_fruits_data.py manquant"
    ((MISSING_SCRIPTS++))
fi
